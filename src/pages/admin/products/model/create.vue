...
onMounted(async () => {
  try {
    ...
    toast.add({ severity: 'error', summary: t("error"), detail: error.response?.data?.message || 'Failed to load brands', life: 3000 });
  }
});
...
const submitForm = async () => {
  ...
  axios.post("/api/model", formData)
    .then(response => {
      router.push({name: 'model'}); // Update this route name as needed
      toast.add({ severity: 'success', summary: t("success"), detail: 'Model created successfully', life: 3000 });
      loading.value = false;
    })
    .catch(error => {
...
<template>
  <div v-can="'create models'" class="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-lg">
    <h1 class="text-3xl font-bold text-center mb-8 text-gray-800">Create New Model</h1>
    ...
    <Form ref="form" @submit.prevent="submitForm" class="space-y-6">
      ...
      <div class="pt-4 flex justify-center space-x-4">
        ...
        <Button
          ...
          :disabled="loading"
        >
          <span v-if="!loading">Create Model</span>
